...
#define NI_DELAY_LABEL        "% Global average NI delay (cycles): "   
#define NW_DELAY_LABEL        "% Global average NW delay (cycles): "   
#define TOTAL_NW_DELAY_LABEL  "% Global Total Nw  avg delay (cycles): "
#define TOTAL_NI_DELAY_LABEL  "% Global Total Ni  avg delay (cycles): "
#define TOTAL_MSG_DELAY_LABEL "% Global Total Msg avg delay (cycles): "

#define TOTAL_LABEL_NUM 7
#define MATLAB_VAR_NAME      "data"
#define MATRIX_COLUMN_WIDTH  15

//---------------------------------------------------------------------------

typedef size_t  uint_new;
...
struct TSimulationResults
{
  ...
  double       ni_delay;
  double       nw_delay;
  double       total_ni_delay;
  double       total_nw_delay;
  double       total_msg_delay;
};
...
{
  for (uint_new i=0; i<indexes.size(); i++)
...
bool RemoveAggregateParameters(TParametersSpace& params_space, 
			       ...
			       string& error_msg)
{
  for (uint_new i=0; i<aggregated_params.size(); i++)
    {
  ...
  if (pv.first == "topology")
    {
      ...
      char times;
      iss >> width >> times >> height >> times >> depth;

      ostringstream oss;
      oss << "-dimx " << width << " -dimy " << height << " -dimz " << depth;
...
string Configuration2CmdLine(const TConfiguration& conf)
{
  ...
  for (uint_new i=0; i<conf.size(); i++)
...
string Configuration2FunctionName(const TConfiguration& conf)
{
  ...
  for (uint_new i=0; i<conf.size(); i++)
    fn = fn + conf[i].first + "_" + conf[i].second + "_";
			   ...
			   string& error_msg)
{
  ...
  for (uint_new i=0; i<explorer_params.size(); i++)
    {
		 ...
		 string& error_msg)
{
  ...
  while (!fin.eof())
	...
	/*
	 pos = line.find(NI_DELAY_LABEL);
      if (pos != string::npos) 
	{
	  nread++;
	  istringstream iss(line.substr(pos + string(NI_DELAY_LABEL).size()));
	  iss >> sres.ni_delay;
	  continue;
	}
	
	pos = line.find(NW_DELAY_LABEL);
      if (pos != string::npos) 
	{
	  nread++;
	  istringstream iss(line.substr(pos + string(NW_DELAY_LABEL).size()));
	  iss >> sres.nw_delay;
	  continue;
	}
	
	pos = line.find(TOTAL_NW_DELAY_LABEL);
      if (pos != string::npos) 
	{
	  nread++;
	  istringstream iss(line.substr(pos + string(TOTAL_NW_DELAY_LABEL).size()));
	  iss >> sres.total_nw_delay;
	  continue;
	}
	
	pos = line.find(TOTAL_NI_DELAY_LABEL);
      if (pos != string::npos) 
	{
	  nread++;
	  istringstream iss(line.substr(pos + string(TOTAL_NI_DELAY_LABEL).size()));
	  iss >> sres.total_ni_delay;
	  continue;
	}
	
	pos = line.find(TOTAL_MSG_DELAY_LABEL);
      if (pos != string::npos) 
	{
	  nread++;
	  istringstream iss(line.substr(pos + string(TOTAL_MSG_DELAY_LABEL).size()));
	  iss >> sres.total_msg_delay;
	  continue;
	}
	*/
    }

  if (nread != TOTAL_LABEL_NUM)
    {
...
bool RunSimulations(double start_time,
		    pair<uint_new,uint_new>& sim_counter,
		    ...
		    string& error_msg)
{
  ...
  for (int i=0; i<repetitions; i++)
	...
	return false;
      ...
      fout << "  ";
      for (uint_new i=0; i<aggr_conf.size(); i++)
	fout << setw(MATRIX_COLUMN_WIDTH) << ExtractFirstField(aggr_conf[i].second); // this fix the problem with pir
	 ...
	 /*  << setw(MATRIX_COLUMN_WIDTH) << sres.ni_delay 
	   << setw(MATRIX_COLUMN_WIDTH) << sres.nw_delay
	   << setw(MATRIX_COLUMN_WIDTH) << sres.total_msg_delay 
	   << setw(MATRIX_COLUMN_WIDTH) << sres.total_ni_delay
	   << setw(MATRIX_COLUMN_WIDTH) << sres.total_nw_delay */
			      ...
			      ofstream& fout, string& error_msg)
{
  ...
  fout << setw(MATRIX_COLUMN_WIDTH) << "avg_delay"
    /*   << setw(MATRIX_COLUMN_WIDTH) << "NI delay"
	   << setw(MATRIX_COLUMN_WIDTH) << "NW delay"
	   << setw(MATRIX_COLUMN_WIDTH) << "All MSG delay"
	   << setw(MATRIX_COLUMN_WIDTH) << "All NI delay"
	   << setw(MATRIX_COLUMN_WIDTH) << "All NW delay"*/
		    ...
		    string&                    error_msg)
{
  ...
  string def_cmd_line;
  for (uint_new i=0; i<default_params.size(); i++)
  ...
  pair<uint_new,uint_new> sim_counter(0, conf_space.size() * aggr_conf_space.size() * eparams.repetitions);
  
  double start_time = GetCurrentTime();
  for (uint_new i=0; i<conf_space.size(); i++)
    {
	...
	return false;
      ...
      for (uint_new j=0; j<aggr_conf_space.size(); j++)
	{
